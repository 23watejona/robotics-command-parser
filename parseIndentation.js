export function parse(str, includeComments = false) {
	...
	for (let match of str.matchAll(m)) {
		if (match.index - (lastIndex + lastLength) != 0) {
			parsed.push({
				tokenType: "text",
				content: str.substring(lastIndex + lastLength, match.index),
			});
		}
		lastIndex = match.index;
		lastLength = match[0].length;
		for (let [captureGroup, string] of Object.entries(match.groups)) {
			if (string != undefined) {
				parsed.push({ tokenType: captureGroup, content: string });
			}
		}
	}
	let res = "";
		...
		if (tokenType == "text") {
			if (indentBuff.length > 0) {
				...
				try {
					...
					//console.log(a)
					res +=
						processIndent(indentBuff[indentBuff.length - 1], lastIndent) + "\n";
				} catch (e) {
			...
			// Accounts for newlines in a Java block
			...
			// Accounts for newlines in comments
			for (const newline of content.matchAll("\n")) {
				lineCount += 1;
			}
			...
			lineCount += 1;
			res += "\n";
		} else if (tokenType == "trailingWhitespace") {
			for (const newline of content.matchAll("\n")) {
				lineCount += 1;
			}
		}
...
function processIndent(currIndent, lastIndent) {
	...
	if (currIndent.replaceAll(tabRegex, "") != "") {
		console.log(JSON.stringify(currIndent))
